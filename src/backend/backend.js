...
async function updateUser(id, token, body) {
	const { isOnline, photos, profilePictureURL } = body;
	const payload = {};
	payload.user_id = id;
	if (isOnline || isOnline === 0) payload.isOnline = Number(isOnline);
	if (photos) payload.photos = JSON.stringify(photos);
